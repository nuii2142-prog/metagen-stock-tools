...
**อัป CSV ให้ Adobe Stock แล้วบางไฟล์ไม่เข้า (ชื่อไฟล์มี `[ ]`)?** Adobe Stock จะข้ามแถวที่ชื่อไฟล์มี
วงเล็บเหลี่ยมแบบเงียบๆ (พบบ่อยกับไฟล์จาก Adobe Firefly เช่น `Firefly_---[Wildlife portrait] ... .jpeg`)
เพราะ Adobe เก็บชื่อไฟล์ที่อัปโหลดไว้ตามเดิม แก้ที่ CSV อย่างเดียวไม่พอ — ลากโฟลเดอร์รูปมาวางบน
**`metagen/Clean Adobe filenames.bat`** จะโชว์ตัวอย่างให้ดูก่อน กด Enter ยืนยัน มันจะตัด `[...]`
ออกทั้งจากชื่อไฟล์รูปและคอลัมน์ Filename ใน CSV ให้ตรงกัน
...
│   ├── Clean Adobe filenames.bat    ← แก้ปัญหา CSV ไม่เข้าเมื่อชื่อไฟล์มี [ ] (ลากวางแล้วใช้ได้เลย)
│   └── clean_adobe_filenames.py     ← สคริปต์เครื่องยนต์ที่ .bat เรียกใช้
├── dreamstime-tool/
